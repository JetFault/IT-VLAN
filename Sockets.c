...
int encapsulate_packet(char* source_ip)
{
	...
	struct packet_header header1;
	struct packet_header *header_p;
	Unsigned int src,dst;
	
	
	//get the source IP address
	header1.source = htonl(/*hex representation of IP address*/);
	header1.destination = htonl(/*hex representation of IP address*/);
...
int tcp_listener(int listenfd, int argc, char* argsv)
{
	...
	if(argc == 3)
	{
		if(/*program is in server mode (AKA the machine running the program is to act as the server*/)
		{
